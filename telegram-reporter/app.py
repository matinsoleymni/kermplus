        ...
        try:
            ...
            try:
                ...
                if "USER_DEACTIVATED" in str(e):
                    logger.error(f"Main bot session is deactivated! {e}")
                else:
                    raise e
            ...
            # 3. اجرای ایجنت‌ها (اکانت‌ها) با هندل کردن خطای دیلیت اکانت
            try:
                await self.agent.launcher()
            except Exception as e:
                if "USER_DEACTIVATED" in str(e):
                    logger.warning(f"One or more agent sessions are deactivated/banned. Bypassing crash... Error: {e}")
                else:
                    raise e
