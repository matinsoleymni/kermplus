...
class AgentManager:
    ...
    async def _launch(
        ...
        try:
            ...
            print(f"[Deactivated/Banned] {agent.name} deleted from DB. Error: {e.__class__.__name__}")
            ...
            # تغییر کلیدی: اگر نوع ارور با ایمپورت‌های بالا مچ نشد، متن ارور رو چک می‌کنیم
            if "USER_DEACTIVATED" in str(e) or "USER_DELETED" in str(e) or "SESSION_REVOKED" in str(e):
                print(f"[Deactivated/Banned - Fallback] {agent.name} deleted from DB. Error: {e}")
                await self.app.database.Session.filter(pk=agent.session_id).delete()
            else:
                # ارورهای ناشناخته فقط پرینت میشن و باعث کرش نمیشن
                print(f"[Unhandled Error on {agent.name}] {e}")
